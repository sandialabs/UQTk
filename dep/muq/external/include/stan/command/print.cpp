...
int calculate_size(const Eigen::VectorXd& x, const std::string& name, const int digits, std::ios_base::fmtflags& format)
{
  using std::max;
  using std::ceil;
  using std::log10;
  ...
  if (fixed_size < 7) {
    return max(fixed_size, max(name.length(), std::string("-0.0").length()) + 0.0);
  }
  ...
  if (x.minCoeff() < 0) {
    exponent_size = max(exponent_size, ceil(log10(log10(-x.minCoeff()))));
  }
...
void print_usage()
{
  std::cout << "  USAGE:  print <filename 1> [<filename 2> ... <filename N>]" << std::endl << std::endl;
}
...
int main(int argc, const char *argv[])
{
  ...
  headers << "mean", "se_mean", "sd", "2.5%", "25%", "50%", "75%", "97.5%", "n_eff", "Rhat";
  ...
  std::cout << "Inference for Stan model: " << model_name << std::endl << chains.num_chains() <<
  " chains: each with iter=(" << chains.num_kept_samples(0);
  ...
  std::cout << ")";
  std::cout << "; " << chains.num_samples() << " iterations saved." << std::endl << std::endl;
  ...
  "and Rhat is the potential scale reduction factor on split chains (at " << std::endl << "convergence, Rhat=1)." <<
  std::endl << std::endl;
  return 0;
}
